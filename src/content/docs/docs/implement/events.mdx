...
A `monetization` event fires when an outgoing payment is successfully created by the visitor's wallet. Outgoing payment requests are sent by the 
...
<Tooltip content="" client:load></Tooltip>

<Tooltip content="" client:load></Tooltip>


<mark>Web Monetization agent</mark> to the visitor's wallet by calling the Open Payments <LinkOut href="https://openpayments.guide/apis/resource-server/operations/create-outgoing-payment/">`outgoing-payments`</LinkOut> endpoint. 
