...
A single HTML document can contain multiple monetization `<link>` elements; however, your site visitor's frontend software determines how to handle this scenario. One visitor's software could split payments evenly between all links while another only sends payments to the first link it finds. 
...
:::tip[Recommendation]
The Interledger WM extension splits payments evenly between all links, <mark>regardless of the payment being a tip or a stream</mark>. We recommend other software developers follow this approach.
:::
