...
The `MonetizationEvent` interface describes the [`monetization`](/docs/implement/events) event that fires when an outgoing payment is successfully created.  
...
By listening for `monetization` events, you can use the information returned by the following properties to find out basic payment details.

* [amountSent](/docs/implement/events#amountsent) - The currency amount and code of the sent payment. Contains two variables, `value` and `currency`, constructed by the `MonetizationCurrencyAmount` interface.
* [incomingPayment](/docs/implement/events#incomingpayment) - A URL you can use to determine if a payment was received and, if so, the amount.
* [paymentPointer](/docs/implement/events#paymentpointer) - The URL of the wallet address that the incoming payment request was sent to.
